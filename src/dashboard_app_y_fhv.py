...
PATHS = {
    ...
    "Yellow Taxi": {
        "trip_features":
            PROCESSED_DIR / "yellow_trip_features.parquet",

        "hourly_demand":
            PROCESSED_DIR / "yellow_hourly_demand.parquet",

        "zone_hourly_demand":
            PROCESSED_DIR / "yellow_zone_hourly_demand.parquet",

        "duration_model":
            OUTPUTS_DIR / "yellow_duration_model.joblib",

        "duration_metrics":
            OUTPUTS_DIR / "yellow_duration_model_metrics.txt",
        ...
        "demand_predictions":
            OUTPUTS_DIR / "yellow_demand_predictions.parquet",

        "zone_demand_model":
            OUTPUTS_DIR / "yellow_zone_demand_model.joblib",

        "zone_demand_predictions":
            OUTPUTS_DIR / "yellow_zone_demand_predictions.parquet",
    ...
    "FHV": {
        ...
        "duration_metrics":
            OUTPUTS_DIR / "fhv_duration_model_metrics.txt",
...
def get_duration_reference(
    ...
    """
    Get an automatic distance estimate for the selected route.

    Yellow Taxi:
        Uses the processed Yellow Taxi trip data. The preferred estimate is
        the median trip_distance for the selected pickup/drop-off route and
        pickup hour. If that route/hour is unavailable, it falls back to the
        route across the full year, and finally to the pickup-zone median.

    FHV:
        FHV does not contain trip_distance. Therefore, its displayed distance
        is estimated from Yellow Taxi historical trips for the same route.
        This distance is display-only; the FHV duration model does not use it.
    """

    yellow_path = PATHS["Yellow Taxi"]["trip_features"]
    ...
    # ------------------------------------------------------------------------
    # 1. Preferred: same route + same pickup hour
    # ------------------------------------------------------------------------
    query_route_hour = f"""
        SELECT
            median(trip_distance) AS distance
        FROM read_parquet('{yellow_path.as_posix()}')
        WHERE PULocationID = {pu_id}
          AND DOLocationID = {do_id}
          AND pickup_hour = {pickup_hour}
          AND trip_distance > 0
          AND trip_distance < 100
    """

    try:
        result = duckdb.sql(query_route_hour).fetchone()
        if result and result[0] is not None:
            return {
                "distance": float(result[0]),
                "source": "historical route/hour",
            }
    except Exception:
        pass

    # ------------------------------------------------------------------------
    # 2. Fallback: same route across the full year
    # ------------------------------------------------------------------------
    query_route = f"""
        SELECT
            median(trip_distance) AS distance
    ...
    try:
        result = duckdb.sql(query_route).fetchone()
        if result and result[0] is not None:
            return {
                "distance": float(result[0]),
    ...
    # ------------------------------------------------------------------------
    # 3. Final fallback: pickup-zone historical median.
    #
    # This guarantees that the UI can still show an estimated distance even
    # when the exact pickup/drop-off pair has no recorded Yellow trips.
    # ------------------------------------------------------------------------
    query_pickup_zone = f"""
        SELECT
            median(trip_distance) AS distance
    ...
    try:
        result = duckdb.sql(query_pickup_zone).fetchone()
        if result and result[0] is not None:
            return {
                "distance": float(result[0]),
                "source": "pickup-zone estimate",
            }
    except Exception:
        pass
...
def predict_duration_tab():
    ...
    st.caption(
        ...
        "Yellow Taxi or FHV duration model."
    )
    ...
    # Same-zone diagnostic / user-facing trip classification.
    ...
    #
    # Yellow: distance is a model input and is obtained automatically.
    # FHV: distance is estimated for display only because FHV has no distance
    # field and the FHV model was trained without distance.
    # ------------------------------------------------------------------------
    ...
    if vehicle == "Yellow Taxi":
        if reference is None:
            st.error(
                "A trip-distance estimate could not be calculated for the "
                "selected pickup zone."
            )
            return

        known = {
            "trip_distance": reference["distance"],
            "passenger_count": 1,
            "PULocationID": pu_id,
            "DOLocationID": do_id,
            "RatecodeID": 1,
    ...
    if reference is not None:
        ...
        if vehicle == "FHV":
            st.caption(
                "Distance is an estimate based on historical Yellow Taxi "
                "trips for the selected route. It is shown for reference only."
            )
        elif reference["source"] != "historical route/hour":
            st.caption(
                f"Distance estimated using {reference['source']} data."
            )
    ...
    # Do not display model-input warnings in the normal prediction UI.
    # The model input remains available inside the optional expander.
...
def page_predict():
    ...
    st.caption(
        ...
        "Yellow Taxi and FHV machine-learning models."
    )
...
def page_dashboard():
    ...
    st.caption(
        "Performance of all six trained models: "
        "Yellow Taxi and FHV."
    )
